...
Player::Player(const defs::Keys& info): server(info) {
    __init_villages();
}

void Player::__init_villages(void) {
    server.update_page(list);

    CDocument* html = server.get_parsed();
    CSelection tds = html->find("td[class='vil fc']");

    for (int i = 0; i < tds.nodeNum(); ++i) {
        CSelection name = tds.nodeAt(i).find("a");
        std::string link = name.nodeAt(0).attribute("href");

        std::string idx = link.substr(village.size());
        server.update_page(village+idx);

        villages.push_back(Village(&server, idx));
    }

    current_village_id = 0;
    server.update_page(village+villages[current_village_id].get_id());
}
